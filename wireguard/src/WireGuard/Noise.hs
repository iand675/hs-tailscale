...
WireGuard uses the Noise_IKpsk2 pattern:
...
* @psk@ = preshared key mixed in after message 2

The implementation follows the Noise Protocol Framework specification:
...
-- | The hash of the protocol name (for Noise_IKpsk2)
-- This is HASH(protocolName) when len(protocolName) > HASHLEN
constructionHash :: Hash
constructionHash = hash protocolName

-- | The identifier hash (hash of construction || "WG_LABEL_MAC1")
-- Used for computing MAC1 in WireGuard
identifierHash :: Hash
identifierHash = hash (hashToBytes constructionHash <> "WG_LABEL_MAC1")

-- | Initialize handshake state with protocol name
initializeHandshake :: ByteString -> HandshakeState
initializeHandshake protoName =
  let h =
        if BS.length protoName <= hashLen
          then protoName <> BS.replicate (hashLen - BS.length protoName) 0
          else hashToBytes $ hash protoName
      ck = h
   in HandshakeState
...
mixHash hs input =
  let newHash = hashToBytes $ hash (hsHash hs <> input)
...
mixKey hs dhResult =
  let (ck, k) = kdf2 (hsChainingKey hs) dhResult
   in hs{hsChainingKey = symmetricKeyToBytes ck, hsSymmetricKey = k}

-- | Mix the preshared key (produces temp hash material and new key)
mixKeyAndHash :: HandshakeState -> ByteString -> HandshakeState
mixKeyAndHash hs psk =
  let (ck, temp, k) = kdf3 (hsChainingKey hs) psk
      newHash = hashToBytes $ hash (hsHash hs <> symmetricKeyToBytes temp)
   in hs{hsChainingKey = symmetricKeyToBytes ck, hsHash = newHash, hsSymmetricKey = k}
...
initiatorInit localStatic remoteStatic psk =
  let hs0 = initializeHandshake protocolName
      ...
      hs2 = mixHash hs1 (publicKeyToBytes remoteStatic)
   in hs2
...
initiatorWriteMessage hs0 ephemeral timestamp =
  let ephemeralPub = derivePublicKey ephemeral
      ...
      hs2 = mixHash hs1 (publicKeyToBytes ephemeralPub)

      -- es: DH(ephemeral, remote_static)
      remoteStatic = case hsRemoteStatic hs2 of
        Just rs -> rs
        Nothing -> error "initiatorWriteMessage: remote static key not set"
      ...
      (hs4, encryptedStatic) = encryptAndHash hs3 (publicKeyToBytes localStaticPub)
      ...
      -- Construct message
      message = publicKeyToBytes ephemeralPub <> encryptedStatic <> encryptedTimestamp
   in (hs6, message)
...
initiatorReadMessage hs0 message
  ...
  | otherwise =
      let (ephemeralBytes, rest) = BS.splitAt 32 message
          ...
          localEphemeral = case hsLocalEphemeral hs2 of
            Just eph -> eph
            Nothing -> error "initiatorReadMessage: local ephemeral key not set"
...
responderInit localStatic psk =
  let hs0 = initializeHandshake protocolName
      ...
      hs2 = mixHash hs1 (publicKeyToBytes localStaticPub)
   in hs2
        { hsLocalStatic = localStatic
        , hsPresharedKey = psk
        }
...
responderWriteMessage hs0 ephemeral =
  let ephemeralPub = derivePublicKey ephemeral
      ...
      -- ee: DH(local_ephemeral, remote_ephemeral)
      remoteEphemeral = case hsRemoteEphemeral hs2 of
        Just eph -> eph
        Nothing -> error "responderWriteMessage: remote ephemeral key not set"
      SharedSecret dhEe = ecdh ephemeral remoteEphemeral
      hs3 = mixKey hs2 (BA.convert dhEe)

      -- se: DH(local_ephemeral, remote_static)
      remoteStatic = case hsRemoteStatic hs3 of
        Just stat -> stat
        Nothing -> error "responderWriteMessage: remote static key not set"
      ...
      -- Construct message
      message = publicKeyToBytes ephemeralPub <> encrypted
