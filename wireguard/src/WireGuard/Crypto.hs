...
import Crypto.Hash (Blake2s_256 (..), hashWith)
...
-- | Create a private key from raw bytes (32 bytes)
privateKeyFromBytes :: ByteString -> Maybe PrivateKey
privateKeyFromBytes bs
  | BS.length bs /= 32 = Nothing
  | otherwise = case X25519.secretKey bs of
      CryptoFailed _ -> Nothing
      CryptoPassed sk -> Just $ PrivateKey sk

-- | Create a public key from raw bytes (32 bytes)
publicKeyFromBytes :: ByteString -> Maybe PublicKey
publicKeyFromBytes bs
  | BS.length bs /= 32 = Nothing
  | otherwise = case X25519.publicKey bs of
      CryptoFailed _ -> Nothing
      CryptoPassed pk -> Just $ PublicKey pk

-- | Create a symmetric key from raw bytes (32 bytes)
symmetricKeyFromBytes :: ByteString -> Maybe SymmetricKey
symmetricKeyFromBytes bs
  | BS.length bs /= 32 = Nothing
  | otherwise = Just $ SymmetricKey bs

-- | Create a preshared key from raw bytes (32 bytes)
presharedKeyFromBytes :: ByteString -> Maybe PresharedKey
presharedKeyFromBytes bs
  | BS.length bs /= 32 = Nothing
  | otherwise = Just $ PresharedKey bs

-- | Convert a private key to raw bytes
privateKeyToBytes :: PrivateKey -> ByteString
privateKeyToBytes (PrivateKey sk) = BA.convert sk

-- | Convert a public key to raw bytes
publicKeyToBytes :: PublicKey -> ByteString
publicKeyToBytes (PublicKey pk) = BA.convert pk

-- | Convert a symmetric key to raw bytes
symmetricKeyToBytes :: SymmetricKey -> ByteString
symmetricKeyToBytes (SymmetricKey bs) = bs

-- | Convert a preshared key to raw bytes
presharedKeyToBytes :: PresharedKey -> ByteString
presharedKeyToBytes (PresharedKey bs) = bs

-- | Convert a hash to raw bytes
hashToBytes :: Hash -> ByteString
hashToBytes (Hash bs) = bs

-- | Convert a nonce to raw bytes
nonceToBytes :: Nonce -> ByteString
nonceToBytes (Nonce bs) = bs
...
-- | All-zero preshared key
zeroPsk :: PresharedKey
zeroPsk = PresharedKey $ BS.replicate 32 0
