...
instance Show LocalClient where
  show lc = "LocalClient { localClientSocketPath = " ++ show (localClientSocketPath lc) ++ " }"
...
localRequest client reqMethod reqPath mBody = do
  result <- try $ do
    initReq <- parseRequest $ "http://local-tailscaled.sock" <> T.unpack reqPath
    let req =
          initReq
            { method = TE.encodeUtf8 reqMethod
            , requestHeaders =
...
localRequestRaw client reqMethod reqPath mBody = do
  result <- try $ do
    initReq <- parseRequest $ "http://local-tailscaled.sock" <> T.unpack reqPath
    let req =
          initReq
...
getTailscaleIPs client = do
  ...
  case result of
    ...
    Right status -> pure $ Right $ fromMaybe [] (statusTailscaleIPs status)
...
getCertDomains client = do
  ...
  case result of
    Left err -> pure $ Left err
    Right status -> pure $ Right $ fromMaybe [] (statusCertDomains status)
...
whoIsNodeKey client key =
  localRequest client "GET" ("/localapi/v0/whois?key=" <> key) Nothing
...
expandSNIName client name = do
  ...
  case result of
    Left err -> pure $ Left err
    Right status -> do
      let suffix = fromMaybe "" (statusMagicDNSSuffix status)
      if T.null suffix
