...
-- | DERP frame types
data FrameType
  = -- | 0x01: Server sends its public key
    FrameServerKey
  | -- | 0x02: Client sends its info
    FrameClientInfo
  | -- | 0x03: Server sends its info
    FrameServerInfo
  | -- | 0x04: Client sends a packet to a peer
    FrameSendPacket
  | -- | 0x05: Server forwards a packet from a peer
    FrameRecvPacket
  | -- | 0x06: Keep-alive frame
    FrameKeepAlive
  | -- | 0x07: Client notes this server is preferred
    FrameNotePreferred
  | -- | 0x08: Server notifies a peer disconnected
    FramePeerGone
  | -- | 0x09: Server notifies a peer connected
    FramePeerPresent
  | -- | 0x0a: Client wants peer notifications
    FrameWatchConns
  | -- | 0x0b: Server tells client to close peer conn
    FrameClosePeer
  | -- | 0x0c: Ping frame
    FramePing
  | -- | 0x0d: Pong frame
    FramePong
  | -- | 0x0e: Health check frame
    FrameHealth
  | -- | 0x0f: Server is restarting
    FrameRestarting
  | -- | 0x10: Forward packet with source info
    FrameForwardPacket
...
data Frame
  ...
  | -- | Packet + source key
    FForwardPacket SendPacket ByteString
  deriving (Eq, Show)
...
data RecvPacket = RecvPacket
  { rpSourceKey :: !ByteString
  -- ^ 32-byte public key of sender
  , rpData :: !ByteString
  -- ^ Packet data
  }
  deriving (Eq, Show)
...
data SendPacket = SendPacket
  { spDestKey :: !ByteString
  -- ^ 32-byte public key of recipient
  , spData :: !ByteString
  -- ^ Packet data
  }
  deriving (Eq, Show)
...
data Restarting = Restarting
  { restartReconnectIn :: !Word32
  -- ^ Milliseconds until reconnect
  , restartTryFor :: !Word32
  -- ^ How long to try reconnecting (ms)
  }
  deriving (Eq, Show)
      ...
      let frameType = BS.index bs 0
       ...
       in if fromIntegral len > maxFrameSize
            then Left "Frame too large"
              ...
              if BS.length bs < 5 + fromIntegral len
