...
data IPv4Header = IPv4Header
  { v4Version     :: !Word8      -- ^ Always 4
  , v4IHL         :: !Word8      -- ^ Header length in 32-bit words
  , v4DSCP        :: !Word8      -- ^ Differentiated Services
  , v4ECN         :: !Word8      -- ^ Explicit Congestion Notification
  , v4TotalLength :: !Word16     -- ^ Total packet length
  , v4ID          :: !Word16     -- ^ Identification
  , v4Flags       :: !Word8      -- ^ Flags (3 bits)
  , v4FragOffset  :: !Word16     -- ^ Fragment offset (13 bits)
  , v4TTL         :: !Word8      -- ^ Time to live
  , v4Protocol    :: !Protocol   -- ^ Protocol
  , v4Checksum    :: !Word16     -- ^ Header checksum
  , v4SrcAddr     :: !Word32     -- ^ Source address
  , v4DstAddr     :: !Word32     -- ^ Destination address
  }
  deriving (Eq, Show)

-- | IPv6 header
data IPv6Header = IPv6Header
  { v6Version     :: !Word8        -- ^ Always 6
  , v6TrafficClass :: !Word8       -- ^ Traffic class
  , v6FlowLabel   :: !Word32       -- ^ Flow label (20 bits)
  , v6PayloadLen  :: !Word16       -- ^ Payload length
  , v6NextHeader  :: !Protocol     -- ^ Next header (protocol)
  , v6HopLimit    :: !Word8        -- ^ Hop limit (TTL equivalent)
  , v6SrcAddr     :: !ByteString   -- ^ Source address (16 bytes)
  , v6DstAddr     :: !ByteString   -- ^ Destination address (16 bytes)
  }
  deriving (Eq, Show)

-- | Parsed IP packet
data IPPacket
  = IPPacketV4 !IPv4Header !ByteString  -- ^ IPv4 packet with payload
  | IPPacketV6 !IPv6Header !ByteString  -- ^ IPv6 packet with payload
  deriving (Eq, Show)
...
data TCPHeader = TCPHeader
  ...
  , tcpSeqNum    :: !Word32
  , tcpAckNum    :: !Word32
  , tcpDataOffset :: !Word8   -- ^ Header length in 32-bit words
  , tcpFlags     :: !Word16   -- ^ Control flags
  , tcpWindow    :: !Word16
  , tcpChecksum  :: !Word16
  , tcpUrgent    :: !Word16
  }
...
serializeIPv4Header IPv4Header{..} = BS.pack
...
serializeIPv6Header IPv6Header{..} =
  let firstWord = (fromIntegral v6Version `shiftL` 28)
