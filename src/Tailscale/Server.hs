  ...
  case statusResult of
    ...
    Right status -> do
      case statusBackendState status of
        StateRunning -> do
          let ips = maybe [] id (statusTailscaleIPs status)
              ipv4 = listToMaybe $ filter isIPv4 ips
              ipv6 = listToMaybe $ filter isIPv6 ips
              certDomains = maybe [] id (statusCertDomains status)
              certDomain = listToMaybe certDomains
              hostname = peerHostName <$> statusSelf status
          pure $ Right TailscaleServerConfig
            { tsAddresses = ips
            , tsIPv4 = ipv4
            , tsIPv6 = ipv6
        ...
        other -> pure $ Left $ LocalAPIHttpError 503 $
...
getTailscaleTLSConfig config = do
  case tsCertDomain config of
    ...
    Just domain -> do
      ...
      case certResult of
        Left err -> pure $ Left err
        Right (CertPair cert key) -> pure $ Right TailscaleTLSConfig
          { tlsCertPEM = cert
          , tlsKeyPEM = key
          , tlsDomain = domain
          }
...
identifyCallerByAddr client remoteAddr = do
  ...
  case result of
    Left _ -> pure Nothing
    Right whois -> pure $ Just CallerInfo
      { callerNodeName = nodeName (whoIsNode whois)
      , callerHostname = maybe "" id $ nodeComputedName (whoIsNode whois)
